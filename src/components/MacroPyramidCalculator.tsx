...
export default function MacroPyramidCalculator() {
  ...
  const totalSize = createMemo(() => entries.reduce((sum, e) => sum + e.size, 0));
  ...
  const totalPnL = createMemo(() => {
    return entries.reduce((sum, e) => {
      const diff = isShort() ? (e.price - currentPrice()) : (currentPrice() - e.price);
  ...
  const generatePyramid = (start: number, interval: number, count: number, sizePerStep: number) => {
  ...
  return (
    <div class="fixed bottom-8 right-8 w-96 bg-zinc-900/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col max-h-[80vh]">
      {/* Header */}
      <div class="p-4 border-b border-white/5 bg-linear-to-r from-indigo-500/10 to-transparent flex items-center justify-between">
        <div>
          ...
          <p class="text-[10px] text-zinc-400 font-mono mt-1">WHALE HUNTER MACRO ENGINE</p>
        </div>
        <button 
          type="button"
      ...
      <div class="p-6 grid grid-cols-3 gap-2 bg-white/2">
        <div class="space-y-1">
          <p class="text-[9px] text-zinc-500 font-mono text-center uppercase tracking-tighter">Current PnL</p>
          <p class={`text-sm font-bold text-center tabular-nums ${totalPnL() >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
            {totalPnL() >= 0 ? '+' : ''}{totalPnL().toFixed(2)}U
          </p>
        </div>
        <div class="space-y-1 border-x border-white/5">
          <p class="text-[9px] text-zinc-500 font-mono text-center uppercase tracking-tighter">Max Risk</p>
          <p class={`text-sm font-bold text-center tabular-nums ${maxRisk() > 0 ? 'text-rose-500' : 'text-zinc-500'}`}>
            {maxRisk().toFixed(2)}U
          </p>
        </div>
        <div class="space-y-1">
          <p class="text-[9px] text-zinc-500 font-mono text-center uppercase tracking-tighter">Locked Profit</p>
          <p class={`text-sm font-bold text-center tabular-nums ${lockedProfit() > 0 ? 'text-emerald-400' : 'text-zinc-500'}`}>
        ...
        <div class="col-span-3 space-y-3 pt-2 border-t border-white/5">
          ...
          <div class="space-y-2">
            <div class="flex justify-between items-center text-[10px] font-mono">
              <span class="text-indigo-400 uppercase tracking-tighter">Target Stop Loss</span>
              <span class="text-rose-400">${stopLoss().toLocaleString()}</span>
            </div>
            <input 
              type="number" 
              value={stopLoss()}
          ...
          <div class="space-y-2 pt-1">
            <div class="flex justify-between text-[10px] font-mono text-zinc-400">
              <span class="uppercase tracking-tighter text-indigo-400/80">Simulated Price</span>
              <span class="text-indigo-400">${currentPrice().toLocaleString()}</span>
            </div>
            <div class="grid grid-cols-5 gap-2 items-center">
              <input 
                type="number" 
                value={currentPrice()}
                onInput={(e) => setCurrentPrice(Number(e.currentTarget.value))}
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar border-t border-white/5">
        <div class="flex items-center justify-between px-1 mb-2">
          <span class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Entry Blocks</span>
        ...
        {showBulk() && (
          <div class="mb-4 space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
            <div class="p-3 bg-indigo-500/5 rounded-xl border border-indigo-500/20">
              <label for="bulk-input-area" class="text-[9px] font-mono text-indigo-400 block mb-2 uppercase tracking-tight">Paste: Price, Size (one per line)</label>
              <textarea 
                id="bulk-input-area"
                value={bulkInput()}
              ...
              <div class="mt-3 flex gap-2">
                <button 
                  type="button"
                  onClick={() => generatePyramid(entries[0]?.price || 70000, 500, 5, 0.04)}
              ...
              <div class="flex items-center gap-3">
                <div class="flex-1 space-y-3">
                  <div class="flex items-center gap-2">
                    <span class="text-[10px] font-mono text-zinc-500">PX:</span>
                    <input 
                      type="number" 
                      value={entry.price}
                      onInput={(e) => updateEntry(entry.id, 'price', Number(e.currentTarget.value))}
                  ...
                  <div class="flex items-center gap-2">
                    <span class="text-[10px] font-mono text-zinc-500">SZ:</span>
                    <input 
                      type="number" 
                      step="0.01"
                      value={entry.size}
                      onInput={(e) => updateEntry(entry.id, 'size', Number(e.currentTarget.value))}
                      class="bg-zinc-800/50 rounded px-2 py-0.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/50 w-full"
                    />
                  </div>
                </div>
                <div class="text-right flex flex-col justify-between h-full">
                  <p class={`text-xs font-mono font-bold ${
                    (isShort() ? (entry.price - currentPrice()) : (currentPrice() - entry.price)) * entry.size >= 0 
                    ? 'text-emerald-500' : 'text-rose-500'
                  }`}>
                    {((isShort() ? (entry.price - currentPrice()) : (currentPrice() - entry.price)) * entry.size).toFixed(2)}U
                  </p>
                  <button 
                    type="button"
      ...
      <div class="p-3 bg-indigo-500/5 flex items-center justify-center gap-4 border-t border-white/5">
        <div class="flex items-center gap-1.5">
          <span class="text-[8px] font-bold text-zinc-500 uppercase">Risk Level</span>
          <div class="w-16 h-1 bg-zinc-800 rounded-full overflow-hidden">
            <div class={`h-full bg-indigo-500 transition-all duration-500`} style={{ width: `${Math.min(totalSize() * 200, 100)}%` }}></div>
          </div>
        </div>
      </div>
    </div>
