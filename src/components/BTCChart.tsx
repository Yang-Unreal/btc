...
export default function BTCChart() {
	...
	const [indicators, setIndicators] = createSignal<Record<string, boolean>>({
		ema20: false, ema60: false, ema120: false, ema150: false, ema200: false,
	});

	const [btcData, setBtcData] = createSignal<BTCData[]>([]);

	// Keep track of last EMA values
	...
	// --- Fetch Data (Initial Load Only) ---
	const fetchHistoricalData = async (activeInterval: Interval): Promise<BTCData[]> => {
		try {
	...
	const mapIntervalToKrakenWS = (interval: Interval): number => {
		const map: Record<string, number> = {
			...
			"1h": 60, "2h": 240, "4h": 240, "12h": 1440,
			"1d": 1440, "3d": 10080, "1w": 10080, "1M": 21600,
		};
	...
	const connectWebSocket = (activeInterval: Interval) => {
		...
		ws.onmessage = (event) => {
			try {
				...
				// Kraken Payload: [channelID, [time, et, o, h, l, c, v, count], channelName, pair]
				if (Array.isArray(data) && data[1] && candlestickSeries) {
					...
					setBtcData((prev) => {
						...
						if (last && last.time === newData.time) return [...prev.slice(0, -1), newData];
						return [...prev, newData];
					});
	...
	const updateIndicatorRealtime = (newData: BTCData) => {
		...
		const updateSeries = (series: ISeriesApi<"Line"> | undefined, lastVal: number | null, setLast: (v: number) => void, period: number) => {
		...
		if (current.ema120) updateSeries(ema120Series, lastEMA120(), setLastEMA120, 120);
		if (current.ema150) updateSeries(ema150Series, lastEMA150(), setLastEMA150, 150);
		if (current.ema200) updateSeries(ema200Series, lastEMA200(), setLastEMA200, 200);
	};
	...
	const loadData = async (activeInterval: Interval) => {
		...
		[ema20Series, ema60Series, ema120Series, ema150Series, ema200Series].forEach(s => {
			s?.setData([]);
		});
	...
	onMount(() => {
		...
		chart.subscribeCrosshairMove((param: MouseEventParams) => {
			...
			setTooltip({
				x: param.point.x, y: param.point.y, time: dateStr,
				open: candle.open.toFixed(2), high: candle.high.toFixed(2), low: candle.low.toFixed(2), close: candle.close.toFixed(2),
				changeColor: candle.close >= candle.open ? "text-teal-600" : "text-red-500",
	...
	createEffect(() => {
		...
		const processEMA = (active: boolean, series: ISeriesApi<"Line"> | undefined, period: number, setLast: (n: number) => void) => {
					...
					<div class="flex flex-wrap gap-3 text-xs">
						{[20, 60, 120, 150, 200].map((p) => (
							<label class="flex items-center gap-1.5 cursor-pointer group select-none">
								<div class={`w-3 h-3 rounded border flex items-center justify-center transition-colors ${indicators()[`ema${p}`] ? "bg-indigo-600 border-indigo-600" : "border-gray-400 bg-white"}`}>
									{/* @ts-ignore */}
									<Show when={indicators()[`ema${p}`]}><svg class="w-2 h-2 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="4" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg></Show>
								</div>
								<input type="checkbox" class="hidden" checked={indicators()[`ema${p}`] ?? false} onChange={(e) => setIndicators((prev) => ({ ...prev, [`ema${p}`]: e.target.checked }))} />
								<span class="text-gray-600 group-hover:text-indigo-600 transition-colors">EMA {p}</span>
							</label>
						))}
					</div>
				</div>
